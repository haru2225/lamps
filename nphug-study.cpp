...
#Hugoniostat（衝撃圧縮）条件下でのNPHアンサンブル（エンタルピー保存）シミュレーションを実行するためのクラス
#
...
#現在の状態（体積・圧力・エネルギー）が Hugoniot 曲線からどれだけずれているかを 温度（K）単位で評価する
...
double FixNPHug::compute_hugoniot()
{
  ...
  e = compute_etotal(); // 現在の全エネルギー

  temperature->compute_vector();


  if (uniaxial == 1) {
    pressure->compute_vector();  // 単軸ならその方向の圧力
 ...
 #force->nktv2p は、圧力の単位変換（逆変換）。たとえば LAMMPS 内部で bar → eV/Å³ へ変換するときに使用
  dhugo = (0.5 * (p + p0 ) * ( v0 - v)) /
    force->nktv2p + e0 - e;   

 #このステップは、ズレ δE を 温度単位（K）に換算
  dhugo /= tdof * boltz;
